...
public class BoatCapabilityClient {
    ...
    public BoatsAllowed getSessionAverage(List<SessionConditions> conditionsThroughoutSession) {
        BoatsAllowed conditionsAtStart = conditionsThroughoutSession.get(0).getBoatsAllowed(); // this is null
    ...
    public BoatsAllowed getBoatsAllowed(OpenWeatherResponse openWeatherResponse) {
        ...
        if (!isTempWithinLimits(weatherData)) {
            ...
            return BoatsAllowed.builder()
                    ...
                    .eight(false)
                    .build();
        } else if (!isGustsWithinLimits(weatherData)) {
            log.info("Gusts too high: {}", (int) Math.round(weatherData.getWindGust()));
            return BoatsAllowed.builder()
                    .doubles(false)
                    .single(false)
                    .quads(false)
    ...
    private boolean isGustsWithinLimits(WeatherData weatherData) {
        int gusts = (int) Math.round(weatherData.getWindGust());
        return gusts <= 30;
    }
